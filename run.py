...
class Runner:
    ...
    def train(self):
        ...
        for iter_i in tqdm(range(res_step)):
            ...
            gradients_sample1 = self.sdf_network.finite_gradient(
                samples, args.precision).squeeze()  # 5000x3
            gradients_sample2 = self.sdf_network.gradient(
                samples).squeeze()       # 5000x3

            grad_norm1 = F.normalize(
                gradients_sample1, dim=1)                # 5000x3
            grad_norm2 = F.normalize(
                gradients_sample2, dim=1)                # 5000x3
            ...
            loss_sdf = torch.linalg.norm(
                (points - sample_moved), ord=2, dim=-1).mean()
